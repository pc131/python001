...
# T322W,T323W,T324R,T331W,T332W,T335R,T336W,T325R,
# T341R,T341W,T342W,T351R,T351W,T352W,T353R,T355R,
# T355W,T356W,T357W,T365R,T501R,T501W,T505R,T505W,
# T551R,T551W,T561R,T561W,T562R,T562W,T563W,T601R,
# T601W,T602W
...
def T213W():
	return wsl_preheader() + '"Transaction":{"'+TRANSACTIONS_XSD_NAMES["T213W"][1]+'":{"Header":{"DataTransaction":"'+TRANSACTIONS_XSD_NAMES["T213W"][0]+'",'+ wsl_postheader() + ',"Payload":{"ORID":"'+global_orid+'","RequestDeferralCode":"'+random.choice(D8229)+'","EffectiveFromDate":"'+today+'","EffectiveToDate":"'+week_ahead+'","AdditionalInformation": "START_DEFERRAL_'+RANDOM_STRING+'"}}}}}}}'
def T214W():
...
def T339R():
	return rtl_preheader() + '"Transaction":{"'+TRANSACTIONS_XSD_NAMES["T339R"][1]+'":{"Header":{"DataTransaction":"'+TRANSACTIONS_XSD_NAMES["T339R"][0]+'",'+ rtl_postheader() + ',"Payload":{"SPID":"'+SPID+'","MissingServiceComponent":"MPW","BusinessType":"Restaurant","ReturntoSewer":"'+returntosewer+'","SurfaceArea":"120","AreaDrained":null,"SewerageSystemConnectedSitePercentage":"30","RelatedRequestORID":null,"AdditionalInformation":"'+fake.paragraph(nb_sentences=1)+'","CustomerContactRequired":"1","RetailerSystemReference": "RET_'+RANDOM_STRING+'","DeclarationDate":"'+today+'","CustomerConsent":{'+CUSTOMER_BLOCK+'}}}}}}}}'
def T339W():
	return rtl_preheader() + '"Transaction":{"'+TRANSACTIONS_XSD_NAMES["T339W"][1]+'":{"Header":{"DataTransaction":"'+TRANSACTIONS_XSD_NAMES["T339W"][0]+'",'+ rtl_postheader() + ',"Payload":{"SPID":"'+SPID+'","MissingServiceComponent":"MPW","BusinessType":"Restaurant","ReturntoSewer":"'+returntosewer+'","SurfaceArea":"120","AreaDrained":null,"SewerageSystemConnectedSitePercentage":"30","RelatedRequestORID":null,"AdditionalInformation":"'+fake.paragraph(nb_sentences=1)+'","DeclarationDate":"'+today+'"}}}}}}}'
def T340W():
	return wsl_preheader() + '"Transaction":{"'+TRANSACTIONS_XSD_NAMES["T342W"][1]+'":{"Header":{"DataTransaction":"'+TRANSACTIONS_XSD_NAMES["T342W"][0]+'",'+ wsl_postheader() + ',"Payload":{"ORID":"'+global_orid+'","CompleteReasonCodeC4":"SCUPDATED","SPIDNotUpdatedDetails":null,"ServiceComponentType":"MPW","ServiceComponentEnabled":"1","AssignedTariffCode":"TARIFF-01","EffectiveFromDate":"'+today+'","BusinessType":"Restaurant","FollowOnRequestORID":null,"ChargeToRetailerFlag":"1","ChargeToRetailerReason":"'+fake.paragraph(nb_sentences=1)+'","AdditionalInformation":"'+fake.paragraph(nb_sentences=1)+'"}}}}}}}'
def T341R():
